...
def main [] {
  run_tests $env.PROCESS_PATH [
    { name: 'test s5cmd should be installed', execute: { test-s5cmd-installed } }
    { name: 'test s5cmd should be able to list objects', execute: { test-s5cmd-list } }
    { name: 'test s5cmd path style should be able to list objects', execute: { test-s5cmd-path-style-list } }
    { name: 'test s5cmd sync should be able to work with hash-only flag', execute: { test-s5cmd-sync-hash-only } }
    # Skipped: test s5cmd sync should be able to work with max-delete flag and skip deletion
    { name: 'test s5cmd sync should be able to work with max-delete flag and delete redundant files', execute: { test-s5cmd-sync-max-delete } }
    { name: 'test s5cmd cp should handle non-regular files with exclude patterns', execute: { test-s5cmd-cp-exclude-nonregular } }
    { name: 'test s5cmd cp should work with client-copy within same S3 endpoint', execute: { test-s5cmd-cp-client-copy } }
  ]
}
...
def test-s5cmd-path-style-list [] {
  load-s3-env stmio
...
def test-s5cmd-sync-max-delete [] {
  sleep 500ms
...
def test-s5cmd-cp-exclude-nonregular [] {
  ...
  if ($test_dir | path exists) { rm -rf $test_dir }
  do -i { s5cmd rm $"($oss_prefix)/test-socket-issue/*" }
}

def test-s5cmd-cp-client-copy [] {
  ...
  # Clean target directory before test (ignore errors if empty)
  do -i { s5cmd rm $"($target_prefix)*" }
  ...
  # Cleanup target files (ignore errors if empty)
  do -i { s5cmd rm $"($target_prefix)*" }
}
