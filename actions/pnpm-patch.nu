...
def regex-escape [input: string]: nothing -> string {
  mut escaped = $input
  for pair in [
    ['.' '\.']
    ['+' '\+']
    ...
    $escaped = ($escaped | str replace -a $pair.0 $pair.1)
  }
  $escaped
}

def ensure-patch-hash-on-leading-version [value: string, pkg_version: string, patch_hash: string]: nothing -> string {
  let hash_suffix = ['(patch_hash=' $patch_hash ')'] | str join
  let escaped_version = regex-escape $pkg_version
  let pattern = ['^(' $escaped_version ')(?:\\(patch_hash=[^)]+\\))?'] | str join
  $value | str replace -ar $pattern (['${1}' $hash_suffix] | str join)
}

def ensure-patch-hash-on-package-ref [value: string, package_ref: string, patch_hash: string]: nothing -> string {
  let hash_suffix = ['(patch_hash=' $patch_hash ')'] | str join
  let escaped_ref = regex-escape $package_ref
  let start_pattern = ['^(' $escaped_ref ')(?:\\(patch_hash=[^)]+\\))?'] | str join
  let peer_pattern = ['([(])(' $escaped_ref ')(?:\\(patch_hash=[^)]+\\))?'] | str join

  $value
    | str replace -ar $start_pattern (['${1}' $hash_suffix] | str join)
    | str replace -ar $peer_pattern (['${1}${2}' $hash_suffix] | str join)
}

def ensure-patch-hash-on-package-refs [value: string, pkg_name: string, pkg_version: string, patch_hash: string]: nothing -> string {
  let pkg_name_plus = $pkg_name | str replace '/' '+'
  let with_name = ensure-patch-hash-on-package-ref $value $'($pkg_name)@($pkg_version)' $patch_hash
  ensure-patch-hash-on-package-ref $with_name $'($pkg_name_plus)@($pkg_version)' $patch_hash
}

def update-dependency-reference-map [
  deps: record,
  pkg_name: string,
  pkg_version: string,
  patch_hash: string
]: nothing -> record<updated: bool, value: record> {
  mut next_deps = $deps
  mut updated = false

  for dep_name in ($deps | columns) {
    let dep_value = $deps | get $dep_name
    if (($dep_value | describe) != 'string') {
      continue
    }

    let next_value = if $dep_name == $pkg_name {
      ensure-patch-hash-on-leading-version $dep_value $pkg_version $patch_hash
    } else {
      ensure-patch-hash-on-package-refs $dep_value $pkg_name $pkg_version $patch_hash
    }

    if $next_value != $dep_value {
      $next_deps = ($next_deps | upsert $dep_name $next_value)
      $updated = true
    }
  }

  { updated: $updated, value: $next_deps }
}

def update-lockfile-entry-reference-sections [
  entry: record,
  pkg_name: string,
  pkg_version: string,
  patch_hash: string
]: nothing -> record<updated: bool, value: record> {
  mut next_entry = $entry
  mut updated = false

  for dep_field in $LOCKFILE_DEP_FIELDS {
    if $dep_field not-in ($entry | columns) {
      continue
    }

    let dep_result = update-dependency-reference-map ($entry | get $dep_field) $pkg_name $pkg_version $patch_hash
    if $dep_result.updated {
      $next_entry = ($next_entry | update $dep_field $dep_result.value)
      $updated = true
    }
  }

  { updated: $updated, value: $next_entry }
}

def inject-patch-hash-to-lockfile [
  lock_data: record,
  pkg_name: string,
  pkg_version: string,
  patch_hash: string
  ...
  if not ($importers | is-empty) {
    mut next_importers = $importers
    mut importers_updated = false

    for importer_id in ($importers | columns) {
      let importer = $importers | get $importer_id
      mut next_importer = $importer
      mut importer_updated = false

      for dep_field in $IMPORTER_DEP_FIELDS {
        if $dep_field not-in ($importer | columns) {
          continue
        }

        let deps = $importer | get $dep_field
        mut next_deps = $deps
        mut deps_updated = false

        for dep_name in ($deps | columns) {
          let dep_entry = $deps | get $dep_name
          let dep_kind = $dep_entry | describe
          let current_version = if ($dep_kind | str starts-with 'record') {
            $dep_entry | get -o version | default ''
          } else if $dep_kind == 'string' {
            $dep_entry
          } else {
            ''
          }
          ...
          let next_version = if $dep_name == $pkg_name {
            ensure-patch-hash-on-leading-version $current_version $pkg_version $patch_hash
          } else {
            ensure-patch-hash-on-package-refs $current_version $pkg_name $pkg_version $patch_hash
          }
          ...
          let next_entry = if ($dep_kind | str starts-with 'record') {
            $dep_entry | upsert version $next_version
          } else {
            $next_version
          }

          $next_deps = ($next_deps | upsert $dep_name $next_entry)
          $deps_updated = true
        }

        if $deps_updated {
          $next_importer = ($next_importer | update $dep_field $next_deps)
          $importer_updated = true
        }
      }

      if $importer_updated {
        $next_importers = ($next_importers | upsert $importer_id $next_importer)
        $importers_updated = true
      }
    }

    if $importers_updated {
      $next_lock = ($next_lock | upsert importers $next_importers)
      $updated = true
    }
  }

  let packages = $next_lock | get -o packages | default {}
  if not ($packages | is-empty) {
    mut next_packages = $packages
    mut packages_updated = false

    for pkg_key in ($packages | columns) {
      let pkg_entry = $packages | get $pkg_key
      let pkg_result = update-lockfile-entry-reference-sections $pkg_entry $pkg_name $pkg_version $patch_hash
      if $pkg_result.updated {
        $next_packages = ($next_packages | upsert $pkg_key $pkg_result.value)
        $packages_updated = true
      }
    }

    if $packages_updated {
      $next_lock = ($next_lock | upsert packages $next_packages)
      $updated = true
    }
  }

  let snapshots = $next_lock | get -o snapshots | default {}
  if not ($snapshots | is-empty) {
    mut next_snapshots = {}
    mut snapshots_updated = false

    for snapshot_key in ($snapshots | columns) {
      let snapshot_entry = $snapshots | get $snapshot_key
      let next_snapshot_key = ensure-patch-hash-on-package-refs $snapshot_key $pkg_name $pkg_version $patch_hash
      let snapshot_result = update-lockfile-entry-reference-sections $snapshot_entry $pkg_name $pkg_version $patch_hash
      let next_snapshot_entry = $snapshot_result.value

      if ($next_snapshot_key != $snapshot_key) or $snapshot_result.updated {
        $snapshots_updated = true
      }

      if ($next_snapshots | columns | any {|it| $it == $next_snapshot_key }) and (($next_snapshots | get $next_snapshot_key) != $next_snapshot_entry) {
        err $'Error: snapshot key conflict detected while updating `($next_snapshot_key)`'
        exit 1
      }

      $next_snapshots = ($next_snapshots | upsert $next_snapshot_key $next_snapshot_entry)
    }

    if $snapshots_updated {
      $next_lock = ($next_lock | upsert snapshots $next_snapshots)
      $updated = true
    }
  }

  { updated: $updated, lock: $next_lock }
}
...
def check-dependencies [] {
  ...
  print $'Current pnpm version: (ansi g)($pnpm_version)(ansi rst)'

  if not (($node_version starts-with 'v20.') and ($pnpm_major == 9)) {
...
def update-lock-hash [
  ...
  let patch_result = upsert-root-patched-dependency-record $lock_data $patch_key $new_hash $patch_path
  mut next_lock = $patch_result.lock
  mut updated = $patch_result.updated

  if not $patch_result.updated {
    success $'Hash unchanged: ($new_hash)'
  } else if ($patch_result.old_hash | is-empty) {
    success $'Added new patch entry: ($patch_key)'
  } else {
    success $'Updated hash: ($patch_result.old_hash) -> ($new_hash)'
  }

  if ($pkg_version | is-not-empty) {
    let inject_result = inject-patch-hash-to-lockfile $next_lock $pkg_name $pkg_version $new_hash
    if $inject_result.updated {
      $next_lock = $inject_result.lock
      $updated = true
  ...
  write-lockfile-via-pnpm-bundle $lock_file $next_lock
}
