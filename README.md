...
### 15. Git 请求代理{#git-proxy}

**功能描述**: 开启或者关闭 git 代理, 目前仅支持在阿里郎加速模式下开启 git 代理

**命令格式**: `just git-proxy status=('on')`
...
- 通过 `just show-env` 命令也可以看到当前 git 代理状态是开启还是关闭(On/Off)；
...
### 18. Git Push 自动将代码同步到多个仓库{#git-sync-branch}
...
- 如果在同步的时候想采用强制推送策略需要：`FORCE=1 just trigger-sync ...`；
...
just trigger-sync feature/sync
```

---

### 20. Homebrew 镜像加速{#brew-speed-up}
...
### 22. 给标品源码仓库批量打 Tag{#gaia-release}
...
# 删除`v2.2.0`对应的当天的Tag
just tag-redev v2.2.0 master b2c,b2b,mbr,pik true
...
### 24. 查询二开仓库的远程分支及 Tag 信息{#ls-redev-refs}
...
**命令格式**: `just ls-redev-refs group=('b2c,b2b,mbr,pik') showBranch=('false')`
...
just ls-redev-refs b2c,b2b true
```

---

### 25. 批量更新远程二开仓库代码到本地{#pull-redev}
...
**命令格式**: `just pull-redev branch=('master') group=('b2c,b2b,mbr,pik') diff=('false')`
...
### 26. 扫描(清理)同步仓库里面冗余分支{#prune-branches}
...
**命令格式**: `just prune-synced-branches`
