...
此元数据同步 TUI 工具本身的实现还是挺复杂的，但是 `Nushell` 却可以轻松驾驭，甚至不需要依赖 `curl`, `jq` 等二进制文件，这是 `Bash` 所无法做到的。开发 TUI 类 CLI 工具是 `Nushell` 的典型使用场景之一，从这个角度来看 `Nushell` 相当于 `TS + HTML + CSS`, 因为其自身具备实现逻辑开发、输出布局、简单的样式控制能力。所以这种脚本语言完全值得你去花时间学习下。
