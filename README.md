...
- `--override` 或 `-o`  该参数的格式为 JS Object, 用于覆盖部署配置里面的同名配置项, 仅在部署时有效，比如已有部署目标 `prod` 对应的部署分支是 `master`, 但是你需要部署 `hotfix/abc` 分支，但是这是个临时分支如果为部署这个分支去改配置还是比较麻烦的，此时可以通过 `t dp prod -o {branch: 'hotfix/abc'}` 来部署，表示除了分支不一样外其他的部署配置跟 `prod` 保持一致。该参数在 `sh/bash/fish/nushell` 下可以正常使用，但是 `zsh` 因为 `{}` 解析问题貌似不支持
- `--help` 或者 `-h` 查看帮助信息
