...
### 22. Git Commit 批量 cherry-pick {#git-batch-pick}
...
### 23. 从命令行执行 Erda 流水线{#run-pipeline}
...
### 24. 从命令行查询 Erda 流水线的执行情况{#query-pipeline}
...
### 25. 在手机端执行或者查询 Erda 流水线{#erda-pipeline-in-mobile}
...
### 27. 在 Erda 流水线里通过钉钉机器人群发消息{#dingtalk-msg-in-erda}
...
### 28. TERP 静态资源云端同步{#terp-assets-transfer}
...
其中 `action` 目前只支持三种 `detect`, `download`, `transfer` & `revert`:
...
# 回滚OSS 云存储里面 dev 挂载点上的 base 前端模块
t ta revert base -t dev -d oss
# 回滚 minio 云存储里面 dev 挂载点上的 base 前端模块, 其中 `alias` 代表 mc 配置的别名
t ta revert base -t dev@alias -d minio
```
...
### 29. TERP 元数据一站式极简同步工具{#meta-data-syncing}
...
- 消费者模式(`art consume`): 比如 `TERP` 团队可以根据上游 `Trantor` 团队给出的制品信息(主要是制品版本号)将 `Trantor` 构建完成的制品下载到本地，然后再上传到 `TERP` 项目里面，并通过该制品创建指定环境(开发/测试/预发/生产)的部署单，然后执行部署并输出部署结果（也可以只创建部署单但是并不部署）, 所有这些操作一条命令即可完成；`consume` 的制品可是项目制品也可以是单一应用制品，如果是单一应用制品则会自动将其包装成项目制品，然后再执行剩下的操作；
...
### 31. EMP 工时填报查询 & 钉钉强力提醒工具{#emp-query-super-notify}
...
### 32. 让 Homebrew 飞起来{#brew-speed-up}
...
### 33. 查询各分支上某依赖的版本及提交信息{#query-deps}
...
### 34. 给标品源码仓库批量打 Tag{#gaia-release}
...
### 36. 查询二开仓库的远程分支及 Tag 信息{#ls-redev-refs}
...
### 38. 扫描(清理)同步仓库里面冗余分支{#prune-branches}
