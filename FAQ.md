...
0. `termix` 使用过程中如果遇到问题该如何处理？

>  使用过程中遇到问题优先考虑用 `t doctor` 来进行自助诊断，如果诊断确实发现了问题可以通过 `t doctor --fix` 来进行修复，修复完毕后如果还有问题可以重启终端试试，有时候如果发现的问题比较多，一个问题的修复可能会依赖其他问题的修复，你可能需要多次运行 `t doctor --fix`，如果每次修复的报错都是同样的就不必重试了，可以在支持群里反馈问题寻求帮助。
>  另外，由于本工具提供了源码、详细的帮助文档和 FAQ 也可以尝试询问 AI 帮忙解决，并将你遇到的错误信息、README.md & FAQ.md 作为输入。
